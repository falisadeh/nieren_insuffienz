...
PATIENT_MASTER_DATA = cs_transfer_path("Original Daten", "Patient Master Data.csv")
HLM_OPERATIONEN_FILE = cs_transfer_path("Original Daten", "HLM Operationen.csv")
AKI_LABEL_FILE = cs_transfer_path("Original Daten", "AKI Label.csv")
DIAGRAM_DIR = cs_transfer_path("Diagramme")

file_path = PATIENT_MASTER_DATA
...
path_patient = PATIENT_MASTER_DATA
path_op = HLM_OPERATIONEN_FILE
...
df_patient["DateOfBirth"] = pd.to_datetime(
    df_patient["DateOfBirth"], errors="coerce", dayfirst=True
)
...
df_merged["Alter_in_Tagen"] = (
    df_merged["Start of surgery"] - df_merged["DateOfBirth"]
).dt.days
...
df_merged["Alter_in_Monaten"] = (
    (df_merged["Alter_in_Tagen"] / 30.44).round().astype("Int64")
)
# Alter in Monaten
...
plt.title(
    "Altersverteilung der Patienten zum OP-Zeitpunkt (in Monaten, 6-Monats-Gruppen)"
)
...
df_merged["Alter_in_Jahren"] = (
    (df_merged["Alter_in_Tagen"] / 365.25).round().astype("Int64")
)
...
df_merged["Start of surgery"] = pd.to_datetime(
    df_merged["Start of surgery"], errors="coerce"
)
...
df_sterbeanalyse = df_op[["PMID", "Start of surgery"]].merge(
    df_patient[["PMID", "DateOfDie"]], on="PMID", how="left"
)

# Berechnung der Differenz in Tagen
df_sterbeanalyse["Tage_bis_Tod"] = (
    df_sterbeanalyse["DateOfDie"] - df_sterbeanalyse["Start of surgery"]
).dt.days
...
df_verstorbene_aki = df_verstorbene.merge(
    df_aki[["PMID", "AKI_Stadium"]], on="PMID", how="left"
)
...
# Merge mit AKI-Daten
df_verstorbene_aki = df_verstorbene_unique.merge(
    df_aki[["PMID", "AKI_Stadium"]], on="PMID", how="left"
)
...
print(
    "Gesamtsumme aller AKI-Stadium-Werte:",
    df_aki_max["AKI_Max_Stadium"].value_counts().sum(),
)
...
# AKI-Verteilung
aki_counts = pd.Series({"AKI 1": 334, "AKI 2": 140, "AKI 3": 73})
...
df_aki = pd.read_csv(AKI_LABEL_FILE, sep=";")
...
df_aki_statistik["Prozent"] = (
    df_aki_statistik["Anzahl Patienten"] / gesamt * 100
).round(1).astype(str) + "%"
