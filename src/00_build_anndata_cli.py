...
def merge_patients(df_ops: pd.DataFrame, df_pat: pd.DataFrame) -> pd.DataFrame:
    """Fügt Patientenattribute (PATIENTS) per PMID an OPs an. Beibehaltung aller OP-Zeilen."""
    ops = df_ops.copy()
    pat = df_pat.copy()

    if "PMID" in ops.columns:
        ops["PMID"] = ops["PMID"].astype(str)
    if "PMID" in pat.columns:
        pat["PMID"] = pat["PMID"].astype(str)

    # Nur zusätzliche Spalten aus PATIENTS anhängen (PMID nicht duplizieren, bestehende in OPS nicht überschreiben)
    add_cols = [c for c in pat.columns if c != "PMID" and c not in ops.columns]
    if add_cols:
        merged = ops.merge(pat[["PMID"] + add_cols], on="PMID", how="left", sort=False)
    else:
        merged = ops
