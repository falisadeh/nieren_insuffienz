...
df_se = df[df["TimestampName"].isin(["Start of surgery", "End of surgery"])][
    keys + ["TimestampName", "Timestamp"]
]

# 3) Pro Eingriff: frühester Start, spätestes Ende
starts = (
    df_se[df_se["TimestampName"] == "Start of surgery"]
    .groupby(keys, as_index=False)["Timestamp"]
    .min()
    .rename(columns={"Timestamp": "Start_supp"})
)
ends = (
    df_se[df_se["TimestampName"] == "End of surgery"]
    .groupby(keys, as_index=False)["Timestamp"]
    .max()
...
ops_valid["OP_ID"] = (
    ops_valid["PMID"].astype(str)
    + "_"
    + ops_valid["SMID"].astype(str)
    + "_"
...
ops_valid.to_csv(BASE / "op_durations_from_supp.csv", sep=";", index=True)
import ehrapy as ep
...
if "SMID" in op:
    op["SMID"] = op["SMID"].astype(str).str.strip()
if "Procedure_ID" in op:
    op["Procedure_ID"] = op["Procedure_ID"].astype(str).str.strip()
for c in ("SMID", "Procedure_ID", "TimestampName"):
    if c in supp:
        supp[c] = supp[c].astype(str).str.strip()
...
def norm_sex(x):
    if pd.isna(x):
        return pd.NA
    s = str(x).strip().lower()
    if s in {"m", "male", "männlich"}:
        return "m"
    if s in {"f", "w", "female", "weiblich"}:
        return "f"
    return pd.NA
...
starts = (
    se[se["TimestampName"] == "Start of surgery"]
    .groupby(keys, as_index=False)["Timestamp"]
    .min()
    .rename(columns={"Timestamp": "Start_supp"})
)
ends = (
    se[se["TimestampName"] == "End of surgery"]
    .groupby(keys, as_index=False)["Timestamp"]
    .max()
    .rename(columns={"Timestamp": "End_supp"})
)
...
ops = op.merge(
    supp_se,
    on=[k for k in ("PMID", "SMID", "Procedure_ID") if k in op.columns],
    how="left",
)

ops["Surgery_Start"] = ops["Start of surgery"].where(
    ops["Start of surgery"].notna(), ops["Start_supp"]
)
ops["Surgery_End"] = ops["End of surgery"].where(
    ops["End of surgery"].notna(), ops["End_supp"]
)

# Dauer
ops["duration_minutes"] = (
...
ops_for_join = (
    ...
    .sort_values(["PMID", "Surgery_Start"])
...
ops_for_join = (
    ...
    .assign(
        ...
        Surgery_Start=lambda d: pd.to_datetime(
            d["Surgery_Start"], errors="coerce"
        ).dt.tz_localize(None),
    )
    .dropna(subset=["Surgery_Start"])
...
akis_for_join = (
    ...
    .assign(
        PMID=lambda d: d["PMID"].astype(str).str.strip(),
        AKI_Start=lambda d: pd.to_datetime(
            d["AKI_Start"], errors="coerce"
        ).dt.tz_localize(None),
    )
    .dropna(subset=["AKI_Start"])
...
master["AKI_linked_0_7"] = (
    (master["AKI_linked"] == 1) & (master["days_to_AKI"].between(0, 7))
).astype("int64")
...
pat_summary = (
    ...
    .agg(
        ...
        AKI_Start=(
            "AKI_Start",
            "max",
        ),  # earliest AKI per prior_map (identisch pro Patient)
    )
    .reset_index()
    .merge(pat[["PMID", "Sex_norm"]], on="PMID", how="left")
)
...
BASE = cs_transfer_path()
path = BASE / "analytic_ops_master.csv"
...
), f"'duration_hours' fehlt. Spalten: {df.columns.tolist()}"
...
obs["AKI_linked_0_7"] = pd.to_numeric(obs["AKI_linked_0_7"], errors="coerce").astype(
    "Int64"
)
...
def get_col(adata, name, dtype=float):
    ...
    raise KeyError(
        f"'{name}' nicht gefunden. obs={list(adata.obs.columns)[:10]}, vars={vars_[:10]}"
    )
...
print(
    ...
    len(prior),
    " | n_f:",
    (prior["Sex_norm"] == "f").sum(),
    "n_m:",
    (prior["Sex_norm"] == "m").sum(),
)
print(
    "Median/IQR f:",
    g.get_group("f").median(),
    g.get_group("f").quantile([0.25, 0.75]).tolist(),
)
print(
    "Median/IQR m:",
    g.get_group("m").median(),
    g.get_group("m").quantile([0.25, 0.75]).tolist(),
)

x = g.get_group("f").dropna()
y = g.get_group("m").dropna()
U, p = mannwhitneyu(x, y, alternative="two-sided")
